...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        self.physicsWorld.contactDelegate = self
            ...
            // Creating the background, bird, ground and scoreLabel
        makebg()
        makeBird()
        makeGround()
        ...
        makePipesTimer = Timer.scheduledTimer(timeInterval: 2, target: self, selector: #selector(GameScene.makePipes), userInfo: nil, repeats: true)
            // This timer didn't need to be instanciated but we use pause it on AppDelegate to prevent multiple pipe creation when the app is on the background
    ...
    func makeBird() {
        ...
        bird.physicsBody?.categoryBitMask = ColliderType.bird.rawValue
            ...
            // Defining the mask that will detect contact (ie: to increase score when there is contact with the gap)
        bird.physicsBody?.contactTestBitMask = ColliderType.object.rawValue
        
            // Defining the mask that will detect collision
    ...
    func makePipes() {
        ...
        pipe2.physicsBody?.collisionBitMask = ColliderType.object.rawValue
            ...
            // Defining the var that will increase the user's score based on contact with the gap between pipes
        let gap = SKNode()
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.categoryBitMask == ColliderType.gap.rawValue || contact.bodyB.categoryBitMask == ColliderType.gap.rawValue {
            ...
            scoreLabel.text = String(score)
                ...
                // By adding this guard statement we prevent the label from being added more than once - resulting in a error
            guard gameOver else {
                gameOver = true
